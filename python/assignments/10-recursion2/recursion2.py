...
#  Description: Assignment 10 | Recursion 2
...
def splitArrayHelper(nums, i, balance):
    if(i == len(nums)):
        return(balance == 0)
    else:
        if(splitArrayHelper(nums, i + 1, balance + nums[i])):
            return(True)
...
def splitOdd10Helper(start, nums, sum1, sum2):
    if(start >= len(nums)):
        return((sum1 % 10 == 0 and sum2 % 2 != 0) or (sum1 % 2 != 0 and sum2 % 10 == 0))
    else:
...
def split53Helper(start, nums, balance):
    if(start >= len(nums)):
        return(balance == 0)
    else:
        if(nums[start] % 5 == 0):
            return(split53Helper(start + 1, nums, balance + nums[start]))
        if(nums[start] % 3 == 0):
            return(split53Helper(start + 1, nums, balance - nums[start]))
        if(split53Helper(start + 1, nums, balance + nums[start])):
            return(True)
        return(split53Helper(start + 1, nums, balance - nums[start]))
